...
do
    ...
    local ConfigsFolder = "oblivion/cfgs"
    local CurrentConfig = nil
    if not isfolder then
        warn("Config system requires writefile/readfile/isfolder functions")
    else
    ...
    function Configs:GetConfigsList()
        ...
        if success and files then
            for _, filePath in ipairs(files) do
                local fileName = filePath:match("([^/]+)$"):gsub(".json$", "")
    ...
    function Configs:SaveConfig(window, configName)
        ...
        local configData = window:GetConfigs()
    ...
    function Configs:LoadConfig(window, configName)
        ...
        if decoded.VERSION ~= window:GetConfigs().VERSION then
            warn("Version mismatch: Config="..tostring(decoded.VERSION)..", Window="..tostring(window:GetConfigs().VERSION))
        end
        
        for tabKey, tabData in pairs(decoded) do
            if tabKey ~= "VERSION" then
                local tab = window:GetTabFromKey(tabKey)
                if tab then
                    tab:LoadConfig(tabData)
                end
            end
        end
    ...
    function Configs:DeleteConfig(configName)
        if not delfile then 
            warn("delfile not available")
    ...
    function Configs:GetCurrentConfig()
        return CurrentConfig
    end

    Airflow.ConfigSystem = Configs
end
...
function Airflow:Init(config)
    ...
    local TabID = #Response.TabConfig + 1
    ...
    local LeftChange = Instance.new("BindableEvent")
    LeftChange:SetAttribute("Value",true)
    local RightChange = Instance.new("BindableEvent")
    RightChange:SetAttribute("Value",true)

    tabObject.Left = Airflow:Elements(ListLeft, LeftChange, {Hightlight = Response.Hightlight})
    tabObject.Right = Airflow:Elements(ListRight, RightChange, {Hightlight = Response.Hightlight})

    function tabObject:AddSection(config)
        config = config or {}
        config.Name = config.Name or "Section"
        config.Position = config.Position or "Left"
        ...
        SectionFrame.Parent = (config.Position == "Left" and ListLeft) or ListRight
        SectionFrame.BackgroundColor3 = Color3.fromRGB(91, 91, 91)
        ...
        Header.Name = Airflow:RandomString()
        Header.Parent = SectionFrame
        Header.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        Header.BackgroundTransparency = 1.000
        Header.BorderColor3 = Color3.fromRGB(0, 0, 0)
        Header.BorderSizePixel = 0
        Header.Position = UDim2.new(0, 10, 0, 7)
        ...
        UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
            ...
            SectionFrame.Size = UDim2.new(1, 0, 0, 30 + UIListLayout.AbsoluteContentSize.Y)
        end)
        
        local SectionChange = Instance.new("BindableEvent")
        SectionChange:SetAttribute("Value",true)
        
        local sectionElements = Airflow:Elements(Content, SectionChange, {Hightlight = Response.Hightlight})
        
        tabObject.Sections[config.Name.."_"..tostring(#tabObject.Sections+1)] = {
            Elements = sectionElements,
            Frame = SectionFrame,
            ChangeEvent = SectionChange
        }
        
        return sectionElements
    end

    function tabObject:GetSectionFromKey(key)
        return tabObject.Sections[key]
    end

    function tabObject:GetConfigs()
        local configs = {
            Left = tabObject.Left:GetConfigs(),
            Right = tabObject.Right:GetConfigs(),
            Sections = {}
        }
        
        for sectionKey, sectionData in pairs(tabObject.Sections) do
            configs.Sections[sectionKey] = sectionData.Elements:GetConfigs()
        end
        
        return configs
    end

    function tabObject:LoadConfig(configData)
        if configData.Left then
            for elementKey, elementValue in pairs(configData.Left) do
                local element = tabObject.Left:GetElementFromKey(elementKey)
                if element and element.API and element.API.SetValue then
                    element.API:SetValue(elementValue.Value)
                end
            end
        end
        
        if configData.Right then
            for elementKey, elementValue in pairs(configData.Right) do
                local element = tabObject.Right:GetElementFromKey(elementKey)
                if element and element.API and element.API.SetValue then
                    element.API:SetValue(elementValue.Value)
                end
            end
        ...
        if configData.Sections then
            for sectionKey, sectionData in pairs(configData.Sections) do
                local section = tabObject.Sections[sectionKey]
                if section then
                    for elementKey, elementValue in pairs(sectionData) do
                        local element = section.Elements:GetElementFromKey(elementKey)
                        if element and element.API and element.API.SetValue then
                            element.API:SetValue(elementValue.Value)
                        end
                    end
    ...
    function tabObject:Disable(value, reason)
        tabObject.Disabled = value
        tabObject.DisableReason = reason or ""
        
        if value then
            DisabledText.Text = reason or "Tab Disabled"
            Airflow:CreateAnimation(DisabledText,0.5,nil,{
                TextTransparency = 0.3
            })
            Airflow:CreateAnimation(ListLeft,0.5,nil,{
                BackgroundTransparency = 0.9
            })
            Airflow:CreateAnimation(ListRight,0.5,nil,{
                BackgroundTransparency = 0.9
            })
        else
            Airflow:CreateAnimation(DisabledText,0.5,nil,{
    ...
    table.insert(Response.TabConfig, tabObject)
    
    Response.TabElements["Tab_"..tostring(TabID)] = function(value)
        if value then
            Response.OpenedTab = function(val)
                if val then
                    LeftChange:SetAttribute("Value",true)
                    LeftChange:Fire(true)
                    RightChange:SetAttribute("Value",true)
                    RightChange:Fire(true)
                    
                    for _, sectionData in pairs(tabObject.Sections) do
                        sectionData.ChangeEvent:SetAttribute("Value",true)
                        sectionData.ChangeEvent:Fire(true)
                    end
                    
                    Airflow:CreateAnimation(ListElemants,0.7,nil,{
                        Position = UDim2.new(0.5,0,0.5,0)
                    })
                else
                    LeftChange:SetAttribute("Value",false)
                    LeftChange:Fire(false)
                    RightChange:SetAttribute("Value",false)
                    RightChange:Fire(false)
                    
                    for _, sectionData in pairs(tabObject.Sections) do
                        sectionData.ChangeEvent:SetAttribute("Value",false)
                        sectionData.ChangeEvent:Fire(false)
                    end
            ...
            Airflow:CreateAnimation(hightlight,0.7,nil,{
                Size = UDim2.new(0, 2, 1, 0)
            })
            
            Airflow:CreateAnimation(InputElement,0.5,nil,{
                BackgroundTransparency = 0.800
            })
            ...
            Airflow:CreateAnimation(Icon,0.5,nil,{
                ImageTransparency = 0
            })
        else
            if Response.OpenedTab then
                Response.OpenedTab(false)
            end
    ...
    Airflow:NewInput(InputElement,function()
        for i,ChaValue in next,Response.TabElements do
            ChaValue(false)
        end
        
        Response.TabElements["Tab_"..tostring(TabID)](true)
    end)

    Airflow:Hover(InputElement)

    return tabObject
end

function Response:CreateConfigTab()
    local configTab = Response:DrawTab({
        Name = "Configs",
        Icon = "settings"
    })
    
    local leftSection = configTab:AddSection({
        Name = "Config Management",
        Position = "Left"
    })
    
    local rightSection = configTab:AddSection({
        Name = "Current Config",
        Position = "Right"
    })
    
    local configList = {}
    local configDropdown = leftSection:AddDropdown({
        Name = "Select Config",
        Values = Airflow.ConfigSystem:GetConfigsList(),
        Default = Airflow.ConfigSystem:GetCurrentConfig() or "None",
        Callback = function(value)
            if value ~= "None" then
                rightSection:GetElementFromKey("Button_LoadConfig_1").API:Visible(true)
                rightSection:GetElementFromKey("Button_DeleteConfig_2").API:Visible(true)
                rightSection:GetElementFromKey("Textbox_ConfigName_3").API:Visible(true)
                rightSection:GetElementFromKey("Button_SaveConfig_4").API:Visible(true)
            else
                rightSection:GetElementFromKey("Button_LoadConfig_1").API:Visible(false)
                rightSection:GetElementFromKey("Button_DeleteConfig_2").API:Visible(false)
                rightSection:GetElementFromKey("Textbox_ConfigName_3").API:SetValue("")
                rightSection:GetElementFromKey("Textbox_ConfigName_3").API:Visible(false)
                rightSection:GetElementFromKey("Button_SaveConfig_4").API:Visible(false)
            end
        end
    })
    
    local refreshButton = leftSection:AddButton({
        Name = "Refresh List",
        Callback = function()
            configList = Airflow.ConfigSystem:GetConfigsList()
            table.insert(configList, "None")
            configDropdown.API:SetValues(configList)
        end
    })
    ...
    rightSection:AddButton({
        Name = "Load Config",
        Callback = function()
            local selected = configDropdown.API:GetValue()
            if selected and selected ~= "None" then
                local success, err = Airflow.ConfigSystem:LoadConfig(Response, selected)
                if success then
                    Airflow:Notify({
                        Title = "Config System",
                        Content = "Config '"..selected.."' loaded successfully!"
                    })
                else
                    Airflow:Notify({
                        Title = "Config System",
    ...
    rightSection:AddButton({
        ...
        Callback = function()
            local selected = configDropdown.API:GetValue()
            if selected and selected ~= "None" then
                local success, err = Airflow.ConfigSystem:DeleteConfig(selected)
                if success then
                    configDropdown.API:SetDefault("None")
                    refreshButton.API:Fire()
                    Airflow:Notify({
                        Title = "Config System",
                        Content = "Config '"..selected.."' deleted successfully!"
                    })
                else
                    Airflow:Notify({
                        Title = "Config System",
    ...
    local configNameBox = rightSection:AddTextbox({
        Name = "Config Name",
    ...
    rightSection:AddButton({
        Name = "Save Config",
        Callback = function()
            local configName = configNameBox.API:GetValue()
            if configName and configName ~= "" then
                local success, err = Airflow.ConfigSystem:SaveConfig(Response, configName)
                if success then
                    refreshButton.API:Fire()
                    Airflow:Notify({
                        Title = "Config System",
